...
import org.xmlrpc.android.MySSLSocketFactory;
		...
		try {
			...
			if(this.httpPreAuth == true) {
				...
				String base64EncodedCredentials = Base64.encodeToString(
					    (username + ":" + password).getBytes("US-ASCII"), Base64.DEFAULT);
				//postMethod.addHeader("Authorization", "Basic " + Base64Coder.encode(auth.getBytes()).toString());
				postMethod.addHeader("Authorization", "Basic " + base64EncodedCredentials.trim());
			}
			...
			if (statusCode != HttpStatus.SC_OK) {
				Log.e("Entomologist", "Statuscode is not OK: " + statusCode);
				switch (statusCode)
				{
				case HttpStatus.SC_FORBIDDEN:
				case HttpStatus.SC_UNAUTHORIZED:
					throw new XMLRPCException("Invalid username and/or password");
				case HttpStatus.SC_NOT_FOUND:
				case HttpStatus.SC_INTERNAL_SERVER_ERROR:
					throw new XMLRPCException("Invalid URL");
				default:
